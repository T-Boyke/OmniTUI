...
# 2. FHD-optimierte Whiptail-StandardgrÃ¶ÃŸen
W_HEIGHT=24
W_WIDTH=95
W_LIST=16
...
# 4. Root-RechteprÃ¼fung Helper
require_root() {
    if [[ $EUID -ne 0 ]]; then
        whiptail --title "Root-Privilegien benÃ¶tigt" \
                 --msgbox "Dieses Skript benÃ¶tigt Administratorrechte (sudo). Bitte starten Sie diesen Vorgang mit sudo." 8 75
        exit 1
    fi
}
